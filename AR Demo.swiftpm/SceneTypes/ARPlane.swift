...
class ARPlane: SCNNode {
    ...
    init(anchor: ARPlaneAnchor, in sceneView: ARSCNView) {
        ...
        if #available(iOS 12.0, *), ARPlaneAnchor.isClassificationSupported {
            let classification = anchor.classification.toString
...
//        }
    ...
    private func makeTextNode(_ text: String) -> SCNNode {
        ...
        textNode.simdScale = SIMD3<Float>(0.001, 0.001, 0.001)
        
        return textNode
    }
    
    func updateExtent(_ anchor: ARPlaneAnchor, _ node: SCNNode) {
        if let extentGeometry = extentNode.geometry as? SCNPlane {
            ...
            extentNode.simdPosition = anchor.center + SIMD3<Float>(node.worldUp * -0.01)
        }
    }
    func updateClassification(_ anchor: ARPlaneAnchor) {
        // Update the plane's classification and the text position
        if #available(iOS 12.0, *),
           let classificationNode = self.classificationNode,
           let classificationGeometry = classificationNode.geometry as? SCNText {
            let currentClassification = anchor.classification.toString
